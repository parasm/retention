...
def facebook_id(id):
	...
	for p in members:
		...
		flash.append({'question':{'type':'image','value':image_url},'answer':{'type':'text','value':p.get('name')},'attempts':0,'stage':0})
	id = flashcards.insert({'time':int(time.time()),'username':profile.get('username'),'cards':flash,'deck_name':group.get('name'),
		'reminded':True,'fb_id':profile.get('id')})
	person = users.find({'fb_id':profile.get('id')}).limit(1)[0]
	person['flashcards'].append({'username':profile.get('username'),'cards':flash,'deck_name':group.get('name'),
